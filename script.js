...
if (bookingForm) {
  bookingForm.addEventListener("submit", async e => {
    ...
    const params = {
      ...
      preferred_time: document.getElementById("ftime")?.value           || "Flexible",
    ...
    try {
      ...
      setLoading(false);
      showStatus("ok", "✅ Booking confirmed! We'll be in touch shortly.");
      bookingForm.reset();
      goToStep(1);
    } catch (err) {
      console.error("EmailJS error:", err);
      setLoading(false);
      showStatus("err", "❌ Failed to send. Please WhatsApp or email us directly.");
    }
  });
}
