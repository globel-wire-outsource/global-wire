/* ================================================================
   ...
   Premium Upgrade: Counters · Multi-Step · Chat · EmailJS
   ================================================================ */
...
function switchTab(tabName, prefillService = null) {
  ...
  // Exit
  outPage.style.cssText = `opacity:0; transform:translateX(${goRight ? "-28px" : "28px"}); transition:opacity 0.35s ease, transform 0.35s ease; pointer-events:none;`;
  ...
  setTimeout(() => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        inPage.style.cssText = `opacity:1; transform:translateX(0); transition:opacity 0.5s cubic-bezier(0.4,0,0.2,1), transform 0.5s cubic-bezier(0.4,0,0.2,1); pointer-events:all;`;
        ...
        setTimeout(() => {
          ...
          // Re-run stats counter when returning to home
          if (tabName === "home") animateStats();

          // Prefill booking service
...
function buildReview() {
  ...
  const fields = [
    ...
    { lbl: "Preferred Date", val: g("fdate") || "Flexible", gold: false },
    { lbl: "Preferred Time", val: document.getElementById("ftime")?.value || "Flexible", gold: false },
...
if (bookingForm) {
  bookingForm.addEventListener("submit", async e => {
    ...
    // Send all common variable aliases so the template works regardless of variable names used
    const params = {
    ...
    try {
      ...
      setLoading(false);
      showStatus("err", `Unable to send — please WhatsApp us on +27 75 432 0577 or email globelwireoutsource@gmail.com`);
    }
  });
}
