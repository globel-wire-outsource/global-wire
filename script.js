...
const EMAILJS_SERVICE = "service_hpyva7l";
...
window.addEventListener("load", () => {
  initEmailJS(); // SDK guaranteed loaded here
  setTimeout(() => {
...
if (bookingForm) {
  bookingForm.addEventListener("submit", async e => {
    ...
    try {
      ...
      setLoading(false);
      showBookingConfirmed(nameVal, serviceVal);
      bookingForm.reset();
      goToStep(1);
    } catch (err) {
...
function generateRef() {
  ...
  return "GW-" + d.getFullYear().toString().slice(-2) + pad(d.getMonth() + 1) + pad(d.getDate()) + "-" + Math.floor(1000 + Math.random() * 9000);
}

function showBookingConfirmed(name, service) {
  // Populate dynamic fields
  document.getElementById("bksName").textContent    = name;
  document.getElementById("bksService").textContent = service;
  document.getElementById("bksRef").textContent     = generateRef();

  // Hide form elements
  if (bookingForm)  bookingForm.style.display  = "none";
  if (stepBar)      stepBar.style.display      = "none";
  const bkAlt = document.querySelector(".bk-alt");
  if (bkAlt)        bkAlt.style.display        = "none";

  // Show & animate success screen
...
function hideBookingConfirmed() {
  ...
  setTimeout(() => {
    ...
    const bkAlt = document.querySelector(".bk-alt");
    if (bkAlt)        bkAlt.style.display        = "";
  }, 400);
}

const bksNewBooking = document.getElementById("bksNewBooking");
if (bksNewBooking) {
  bksNewBooking.addEventListener("click", () => {
    hideBookingConfirmed();
  });
}
